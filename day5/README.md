...
The Thermal Environment Supervision Terminal (TEST) starts by running a _diagnostic program_ (your puzzle input). The TEST diagnostic program will run on your [existing Intcode](https://adventofcode.com/2019/day/2) computer after a few modifications:

First, you'll need to add _two new instructions_:
...
Each parameter of an instruction is handled based on its parameter mode. Right now, your ship computer already understands parameter mode `0`, _position mode_, which causes the parameter to be interpreted as a position - if the parameter is `50`, its value is _the value stored at address `50` in memory_. Until now, all parameters have been in position mode.

Now, your ship computer will also need to handle parameters in mode `1`, immediate mode. In _immediate mode_, a parameter is interpreted as a value - if the parameter is `50`, its value is simply `50`.
...
After providing `1` to the only input instruction and passing all the tests, _what diagnostic code does the program produce_?
...
Your computer is only missing a few opcodes:
  ...
  - Opcode `6` is _jump-if-false_: if the first parameter is _zero_, it sets the instruction pointer to the value from the second parameter. Otherwise, it does nothing.
